...
func main() {
	...
	totalSize = directorySizeMap["/"]
	sizeLeft := 70000000 - totalSize
	minThreshold := 30000000 - sizeLeft
	minDirectorySize := math.MaxInt32

	for _, v := range(directorySizeMap) {
		if v >= minThreshold {
			if v < minDirectorySize {
				minDirectorySize = v
			}
		}
	}

	log.Println(minDirectorySize)
